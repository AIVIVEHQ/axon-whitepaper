...
AXON 采用固定总量 10 亿的设计（发行价与 FDV 暂不对外披露）。代币在 TGE（生成事件）时仅释放 **4%** 进入流通，其余按各桶解锁规则线性释放（见 [7.2 解锁与流通曲线](7-2-vesting-circulation.md)）。
