...
同一模型也用于**带单一键跟单**（[E.3.6](e3-copy-trading.md)）：跟单会话密钥的 $L_{\text{tx}}$ 对应单场额度、$L_{\text{total}}$ 对应等级累计上限、$c_{\text{allow}}$ 限定资金只能进托管/结算合约——面向人类用户的授权，与面向 AI 代理的授权共用同一套有界、定向、可撤销的谓词。
